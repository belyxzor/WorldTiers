...
export default function App(){
 ...
 const page=useMemo(()=>{if(path==='/')return <HomePage navigate={navigate}/>;if(path==='/stats')return <StatsPage/>;if(path==='/tier-tagger')return <TierTaggerPage/>;if(path==='/infos')return <AboutPage/>;if(path==='/admin')return <AdminPage/>;if(path.startsWith('/mode/'))return <ModePage modeId={decodeURIComponent(path.split('/')[2]||'')} navigate={navigate}/>;if(path.startsWith('/player/')){const p=findPlayer(decodeURIComponent(path.split('/')[2]||''));return p?<PlayerPage player={p} navigate={navigate}/>:<NotFoundPage navigate={navigate}/>};return <NotFoundPage navigate={navigate}/>},[path,dataVersion]);
 return <><Header navigate={navigate} theme={theme} onTheme={()=>setTheme(theme==='dark'?'light':'dark')} onInfo={()=>setInfo(true)}/>{page}<Footer navigate={navigate}/>{info&&<InfoModal onClose={()=>setInfo(false)} navigate={navigate}/>}</>;
}
